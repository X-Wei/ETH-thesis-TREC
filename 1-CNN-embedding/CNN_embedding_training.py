...
def embed_w2v():
    return Embedding(input_dim=EMBEDDING_INPUT_DIM ,output_dim=EMBEDDING_DIM, 
              weights=[embedding_w2v],input_length=INPUT_SEQ_LEN, trainable=False, name='embed_w2v')
def embed_glove():
    return Embedding(input_dim=EMBEDDING_INPUT_DIM ,output_dim=EMBEDDING_DIM, 
              weights=[embedding_glove],input_length=INPUT_SEQ_LEN, trainable=False, name='embed_glove')


# ### model 1.1: w2v embedding layers, 1 conv1d layers

model_1conv1d_w2v =Sequential(
        [   embed_w2v(),
        ...
        name = 'model_1conv1d_w2v')

# ### model 1.2: glove embedding layers, 1 conv1d layers
model_1conv1d_glove =Sequential(
        [   embed_glove(),
            Conv1D(256, 5, activation='relu'),
            MaxPooling1D(10),
            Flatten(),
            ...
            Dropout(p=0.5),
            Dense(N_LABELS, activation='sigmoid') ],
        name = 'model_1conv1d_glove')

# ### model 2.0: 2 embedding layers, 2 conv1d layers
input_layer = Input(shape=(INPUT_SEQ_LEN,), dtype='int32', name='main_input')

embed1 = embed_w2v()(input_layer)
conv_embed1 = Conv1D(128, 5, activation='relu')(embed1)

embed2 = embed_glove()(input_layer)
...
model_2conv1d_2embed = Model(input=input_layer, output=output_layer, 
                                 name = 'model_2conv1d_2embed')

# ### model 2.1: use w2v embedding, 2 conv1d layers
model_2conv1d_w2v = Sequential(
        [   embed_w2v(),
            ...
            Dropout(p=0.5),
            Dense(N_LABELS, activation='sigmoid') ], 
       name = 'model_2conv1d_w2v')


# ### model 2.2: use glove instead of w2v embedding
model_2conv1d_glove = Sequential(
        [   embed_glove(),
            Conv1D(128, 5, activation='relu'),
            MaxPooling1D(5),
            Conv1D(128, 5, activation='relu'),
            ...
            Dropout(p=0.5),
            Dense(N_LABELS, activation='sigmoid') ], 
       name = 'model_2conv1d_glove')


# ### model 3: 3 conv layers 
model_3conv1d_w2v =Sequential(
        [   embed_w2v(),
            Conv1D(256, 5, activation='relu'),
            MaxPooling1D(5),
            Conv1D(128, 5, activation='relu'),
            ...
            Dropout(p=0.5),
            Dense(N_LABELS, activation='sigmoid') ],
        name = 'model_3conv1d_w2v')

model_3conv1d_glove =Sequential(
        [   embed_glove(),
            Conv1D(256, 5, activation='relu'),
            MaxPooling1D(5),
            Conv1D(128, 5, activation='relu'),
            ...
            Dropout(p=0.5),
            Dense(N_LABELS, activation='sigmoid') ],
        name = 'model_3conv1d_glove')

if __name__ == '__main__':
    # ## start testing different models 
    flag_quick_test = 0
    compile_fit_evaluate(model_1conv1d_w2v, flag_quick_test)
    compile_fit_evaluate(model_1conv1d_glove, flag_quick_test)
    # compile_fit_evaluate(model_2conv1d_2embed, flag_quick_test)
    compile_fit_evaluate(model_2conv1d_w2v, flag_quick_test)
    compile_fit_evaluate(model_2conv1d_glove, flag_quick_test)
    compile_fit_evaluate(model_3conv1d_w2v, flag_quick_test)
    compile_fit_evaluate(model_3conv1d_glove, flag_quick_test)